...
// Currently the 'en', 'ru' values are supported
define('XLITE_EDITION_LNG', 'ru');

define ('XLITE_INSTALL_MODE', 1);
define('LC_DO_NOT_REBUILD_CACHE', true);

if (version_compare(phpversion(), '5.3.0') < 0) {
    die('X-Cart 5 cannot start on PHP version earlier than 5.3.0 (' . phpversion(). ' is currently used)');
}
...
$modules = array (
	...
	array( // 2
			"name"          => 'cfg_install_db',
			"comment"       => xtr('Configuring X-Cart 5'),
            "auth_required" => true,
			"js_back"       => 1,
			"js_next"       => 1,
		    "remove_params" => array(
...
function processCacheRebuildFailure()
{
...
<td>
  <input id="try-button" name="try_again" type="button" class="btn btn-default btn-lg" value="<?php echo xtr('Try again'); ?>" onclick="javascript:document.ifrm.go_back.value='1'; document.ifrm.current.value='1'; document.ifrm.submit();" />
</td>
...
<td>
  <input id="next-button" name="next_button" type="submit" class="btn btn-warning btn-lg" value="<?php echo xtr('Next'); ?>"<?php echo ($error || $current == get_step('check_cfg') ? ' disabled="disabled"' : ''); ?> onclick="javascript: if (step_next()) { ifrm.submit(); return true; } else { return false; }" />
</td>
